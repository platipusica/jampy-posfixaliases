...
function Events7() { // al1.journals.email_aliases 

	function on_view_form_created(item) {
		var height = $(window).height() - $('body').height() - 20,
			time_out;
		...
		item.aliases.on_after_scroll = function(al) {
			...
			time_out = setTimeout(function() {
					if (al.rec_count) {
						item.open({where: {alias: al.id.value}});
					}
					else {
	...
	function on_before_post(item) {
		...
		item.alias.value = item.aliases.id.value;
	}
	this.on_view_form_created = on_view_form_created;
